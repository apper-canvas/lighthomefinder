...
const loadProperty = async () => {
    ...
    try {
      ...
      // Check if property is saved using collection service
      try {
        const collections = await collectionService.getAll()
        const savedCollection = collections.find(c => c.propertyIds.includes(id))
        setIsSaved(!!savedCollection)
      } catch (collErr) {
        setIsSaved(false)
      }
    } catch (err) {
  ...
  const handleSaveToggle = async () => {
    try {
      if (isSaved) {
        // Remove from saved - find and update collection
        const collections = await collectionService.getAll()
        const savedCollection = collections.find(c => c.propertyIds.includes(id))
        if (savedCollection) {
          await collectionService.removeProperty(savedCollection.id, id)
        }
        setIsSaved(false)
        toast.success('Property removed from saved')
      } else {
        // Add to saved - find or create default collection
        let collections = await collectionService.getAll()
        let savedCollection = collections.find(c => c.name === 'Saved Properties')
        
        if (!savedCollection) {
          savedCollection = await collectionService.create({
            name: 'Saved Properties',
            propertyIds: []
          })
        }
        
        await collectionService.addProperty(savedCollection.id, id)
        setIsSaved(true)
                    ...
                    <div className="border-b border-gray-200 mb-6">
                        <nav className="flex space-x-8">
                            {tabs.map(tab => <button
                                ...
                                className={`flex items-center py-2 px-1 border-b-2 font-medium text-sm ${activeTab === tab.id ? "border-primary text-primary" : "border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300"}`}>
                    ...
                    <div>
                        ...
                        {activeTab === "features" && <motion.div
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                {property.features.map((feature, index) => <motion.div
                                    key={index}
                                    initial={{
                                        opacity: 0,
                                        x: -20
                        ...
                        {activeTab === "location" && <motion.div
                            ...
                            <div className="bg-gray-100 rounded-lg p-6 text-center">
                                ...
                                <p className="text-gray-600 mb-4">View this property's location and explore the neighborhood
                                                          </p>
                ...
                <motion.div
                    ...
                    <div className="space-y-4">
                        <Button
                            ...
                            size="lg"
                            icon="Phone"
                            className="w-full">Contact Agent
                                            </Button>
                        <Button
                            ...
                            size="lg"
                            icon="Calendar"
                            className="w-full">Schedule Tour
                                            </Button>
